...
export const kanaswap = async () => {
  const response = await axios.post(
    batch_local.batchSwapQuote,
    {
      ...
      sourceChainId: [3, 3, 10, 10],
      targetChainId: [2],
      fromAmounts: ["100000000000000000","100000","1000000000000000", "100000"],
      fromAddress: [
        "0x520Bc0f107173901bDfEdb0528BaEA885c585594",
        "0x520Bc0f107173901bDfEdb0528BaEA885c585594",
        "0x520Bc0f107173901bDfEdb0528BaEA885c585594",
        "0x520Bc0f107173901bDfEdb0528BaEA885c585594"
      ],
  ...
  console.log(
    "🚀 ~ kanaswap ~ swapInstruction:",
    swapInstruction.data?.data[0]
  );
  console.log(
    "🚀 ~ kanaswap ~ swapInstruction:",
    swapInstruction.data?.data[0][0]
  );
...
//     txHash: swapInstructionResponce.bridgeTxHash!,
...
type SwapInstruction = {
  approveIX: InstructionObject[];
  swapIX: InstructionObject[];
};

type BridgeInstruction = {
  approveIX: InstructionObject;
  transferIX: InstructionObject;
};

type KanaswapInstruction = {
...
const prepareAndSendTx = async (
  ...
  const gasLimit = _increaseGasLimit(Number(gasEstimate));
  const finalTx = { ...tx, gasLimit };

  const sentTx = await signer.sendTransaction(finalTx);
...
const executeEVMInstruction = async (
  signer: ethers.Wallet,
  instruction: KanaswapInstruction
) => {
  if (!instruction.swapInstruction || !instruction.bridgeInstruction) return {};

  const { approveIX, swapIX } = instruction.swapInstruction;
  const { approveIX: bridgeApproveIX, transferIX } =
    instruction.bridgeInstruction;

  // Step 1: Swap approvals
  for (const approve of approveIX) {
    await prepareAndSendTx(signer, approve);
  }

  // Step 2: Swap execution
  let lastSwapReceipt: ethers.TransactionReceipt | undefined;
  for (const swap of swapIX) {
    lastSwapReceipt = await prepareAndSendTx(signer, swap);
  }

  // Step 3: Bridge approval
  await prepareAndSendTx(signer, bridgeApproveIX);

  // Step 4: Bridge transfer
  const bridgeTransferReceipt = await prepareAndSendTx(signer, transferIX);

  return {
    lastSwapTxHash: lastSwapReceipt?.hash,
    bridgeTxHash: bridgeTransferReceipt?.hash,
  };
};
